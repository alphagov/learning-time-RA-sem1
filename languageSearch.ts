...
const getRepoLanguage = async (owner: string, repoName: string): Promise<repoLanguageResponse> => {
  return {
    [repoName]: ( await
      octokit.request(`GET /repos/${owner}/${repoName}/languages`, {
        headers: {
...
const getRepoLanguagesFromList = async (repoNames: string[]) => await Promise.all(repoNames.map(repoName => {
...
getFilteredRepoLanguages('alphagov', 'txma').then(data => {
  fs.writeFileSync('txma-langs.json', JSON.stringify(data))
})
